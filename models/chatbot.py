...
"""
    ...
    def _perform_security_checks(self, text: str) -> dict:
        ...
        restricted_keywords = [
            'eval', 'exec', 'system', 'import', 'delete', 'drop',
            'update','key', 'modify', 'insert', 'script', '<script>',
            'javascript', 'admin', 'password', 'credential','passkey',
            'password','break', 'forge', 'fabricate', 'fake', 'counterfeit',
            'imitate', 'mimic', 'copy', 'duplicate', 'reproduce', 'clone',
            'emulate', 'mirror', 'reflect', 'echo', 'repeat', 'reiterate',
        ]
        ...
        if security_risks['injection_risk']:
            ...
            security_risks['risk_message'] = self.jailbreak_handler.get_response(attempt_type)
    ...
    def generate_response(self, user_input: str, user_context: str) -> tuple[str, bool]:
        ...
        if security_check['length_risk']:
            return "Please provide a reasonable length query (between 2 and 2000 characters).", False
        ...
        try:
            ...
            ans=["Well...Time is ticking ,hurry and get the passkey","Come on there is isn't much time left ...or are you chickening out","Let's Hurry up and get that passkey"]
            ans2=["Better pick up the pace ,no time to waste","Time waits for no one and includes you too","Time is of the essence and you have none of it, Hurry up"]
            ...
            return f"{result['answer']}\n\nWould you like to explore related topics or check the references?", True
...
class WikiJailbreakResponses:
    ...
    def get_response(self, attempt_type="general"):
        ...
        responses = self.jailbreak_responses.get(attempt_type, self.jailbreak_responses["general"])
    ...
    def detect_attempt_type(self, input_text):
        ...
        if any(keyword in input_lower for keyword in ["select", "insert", "update", "delete", "drop"]):
            return "sql"
        elif any(keyword in input_lower for keyword in ["script", "javascript", "console", "eval"]):
            return "script"
        elif any(keyword in input_lower for keyword in ["ignore", "bypass", "override", "you are", "you're not"]):
            return "prompt"
        else:
            return "general"
